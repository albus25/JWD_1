...
public class UpdEmp extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            e.setDeptID(Integer.parseInt(request.getParameter("deptID")));
            
            int sts = AllMethods.UpdateEmployee(e, eid);
            if(sts>0){
                response.sendRedirect("ShowEmpServlet");
            }
            else{
                out.println("<!DOCTYPE html>");
                out.println("<html>");
                out.println("<head>");
                out.println("<title>Servlet UpdEmp</title>");            
                out.println("</head>");
                out.println("<body>");
